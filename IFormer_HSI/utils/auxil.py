...
def loadData(name, num_components=None):
    ...
    if name == 'IP':
        ...
        data = sio.loadmat(os.path.join(data_path, 'HoustonU_IM.mat'))['hustonu']
        labels = sio.loadmat(os.path.join(data_path, 'HoustonU_gt.mat'))['hustonu_gt']
        # lidar = sio.loadmat(os.path.join(data_path, 'HoustonU_LiDAR.mat'))['imggt'][:, :, None]
        # data = np.concatenate((data, lidar), axis=2)
    else:
        print("NO DATASET")
        exit()
    row,col = np.where(labels != 0)
    sio.savemat('row.mat', {'row': row})
    sio.savemat('col.mat', {'col': col})
    shapeor = data.shape
    data = data.reshape(-1, data.shape[-1])

    # if num_components != None:
    #     data_pca = PCA(n_components=num_components).fit_transform(data)
    ...
    shapeor = np.array(shapeor)
    shapeor[-1] = num_components
    # data_pca = data_pca.reshape(shapeor)
    num_class = len(np.unique(labels)) - 1
    return data, labels, num_class, row, col
